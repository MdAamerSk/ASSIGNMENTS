...
- **Team Profiles:** Interactive cards featuring the expert team with social media links appearing on hover.
- **Responsive Philosophy Grid:** A multi-colored grid layout highlighting brand values and approach.
- **Testimonials Section:** Elegant quote-based layouts for showcasing client feedback.
- **Insights/Blog:** A curated section for branding tips and industry news.
...
- **Flexbox:** Employed for the navigation bar, hero content alignment, and footer structure.
- **Positioning:** Utilized `absolute` and `relative` positioning for the team member social overlays.
- **Keyframe Animations:** Created the infinite horizontal scroll for the partner logo marquee.
- **Hover Transitions:** Implemented smooth `transform: scale` and `opacity` transitions for interactive cards and images.
- **Box Model & Design System:** Consistent use of `padding`, `margin`, and `gap` to maintain a spacious, premium feel.

## Project Structure
```text
Assignment5/
├── assets/         # Contains SVGs for logos, WebP for team/work images
├── index.html      # Main project structure and semantic content
└── style.css      # Custom styles, layout definitions, and animations
```
...
- Add an "Add to Cart" functionality using basic JavaScript.
- Enhance the "Get in Touch" button with a popup modal or form validation.
